...
class PaymentsTrackerPage extends StatefulWidget {
  ...
  const PaymentsTrackerPage({
    ...
    required this.onSaveReminderSettings,
...
class _PaymentsTrackerPageState extends State<PaymentsTrackerPage> {
  ...
  Future<void> _sendReminder(Tenant tenant, Property property) async {
    ...
    if (_reminderChannel == 'WhatsApp' || _reminderChannel == 'All') {
      ...
      url = Uri.parse(
        'mailto:${tenant.email}?subject=Rent Reminder for ${property.name}&body=$encodedMessage',
      );
      ...
      if (await canLaunchUrl(url)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Could not open $_reminderChannel application.'),
            ),
          );
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    final linkedTenant = selectedProperty.currentTenantId != null
        ? widget.tenants.firstWhere(
            (t) => t.id == selectedProperty.currentTenantId,
            orElse: () => widget.tenants.first,
          )
        : null;

    final propertyPayments = widget.payments
        .where(
          (p) => p.entityId == selectedProperty.id && p.entityType == 'Rent',
        )
        .toList();
            ...
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'This property is vacant. Please assign a tenant from the Properties tab to track payments.',
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          'Rent: ₹${selectedProperty.rentAmount.toInt()}',
                          style: const TextStyle(
                            color: Colors.white70,
                            fontSize: 11,
                            fontWeight: FontWeight.bold,
                    ...
                    Row(
                      ...
                      children: [
                        if (selectedProperty.agreementEndDate != null)
                          Text(
                            'Ends: ${selectedProperty.agreementEndDate}',
                            style: const TextStyle(
                              color: Colors.white70,
                              fontSize: 11,
                            ),
                        ...
                        if (selectedProperty.agreementEndDate == null)
                          const SizedBox(),
                        Text(
                          'Deposit: ₹${selectedProperty.depositAmount.toInt()}',
                          style: const TextStyle(
                            color: Colors.white70,
                            fontSize: 11,
                          ),
                  ...
                  ElevatedButton.icon(
                    onPressed: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Preparing rent alert for ${linkedTenant.name} via $_reminderChannel...',
                          ),
                ...
                children: [
                  ...
                  if (_paymentStatus == 'Paid') ...[
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: ['UPI/GPay', 'Net Banking', 'Cash', 'Cheque'].map((
                        md,
                      ) {
                        return DropdownMenuItem<String>(
                          value: md,
                          child: Text(md, style: const TextStyle(fontSize: 12)),
                        );
                      }).toList(),
                      onChanged: (val) {
                  ...
                  ElevatedButton(
                    onPressed: _isSaving
                        ...
                        : () async {
                            ...
                            final alreadyExists = propertyPayments.any(
                              (p) => p.description.contains(_paymentMonth),
                            );
                            if (alreadyExists) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    'A rent entry for $_paymentMonth already exists.',
                                  ),
                            ...
                            final pay = Payment(
                              ...
                              transactionId: _paymentStatus == 'Paid'
                                  ? (_txnController.text.isNotEmpty
                                        ? _txnController.text
                                        : randomId)
                                  : null,
                              description:
                                  'Rent for $_paymentMonth - ${selectedProperty.name} ($_paymentMode)',
                            );
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 4),
                                  Text(
                                    'Date: ${p.paymentDate} | Ref: ${p.transactionId ?? "Pending"}',
                                    style: const TextStyle(
                                      fontSize: 10,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Date of Payment: ${payment.paymentDate}',
            ...
            RichText(
              text: TextSpan(
                ...
                children: [
                  ...
                  TextSpan(
                    text: '₹${payment.amount.toInt()} Only',
                    style: const TextStyle(
                      fontWeight: FontWeight.bold,
                      decoration: TextDecoration.underline,
                    ),
                  ...
                  const TextSpan(
                    text:
                        ' by way of electronic bank transfer towards the rent of ',
                  ),
                  TextSpan(
                    text: property.name,
                    style: const TextStyle(
            ...
            ElevatedButton.icon(
              onPressed: () async {
                ...
                String formattedPhone = tenant.mobile.replaceAll(
                  RegExp(r'\D'),
                  '',
                );
                ...
                try {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          'Redirecting to WhatsApp! (Also visible on Tenant Dashboard)',
                        ),
