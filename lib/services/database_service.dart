...
class DatabaseService extends ChangeNotifier {
  ...
  Future<void> fetchTenantDashboardData(String userMobile) async {
    ...
    try {
      ...
      final tenantResponse = await _supabase.from('tenants').select().eq('mobile', userMobile).maybeSingle();
      ...
      if (tenantResponse != null) {
        ...
        if (currentTenantInfo!.propertyId != null) {
          final propResponse = await _supabase.from('properties').select().eq('id', currentTenantInfo!.propertyId!).maybeSingle();
          if (propResponse != null) {
            currentTenantProperty = Property.fromJson(Map<String, dynamic>.from(propResponse));
          }
  ...
  Future<void> _fetchPaymentsForProperty(String propertyId) async {
    try {
      final response = await _supabase.from('payments').select().eq('entity_id', propertyId).order('created_at', ascending: false);
      payments = (response as List).map((json) => Payment.fromJson(Map<String, dynamic>.from(json))).toList();
    } catch (e) {
  ...
  Future<void> _fetchDocumentsForEntity(String entityId) async {
    try {
      final response = await _supabase.from('documents').select().eq('entity_id', entityId).order('uploaded_at', ascending: false);
      documents = (response as List).map((json) => Document.fromJson(Map<String, dynamic>.from(json))).toList();
    } catch (e) {
  ...
  Future<void> _fetchProperties(String ownerId) async {
    ...
    properties = (response as List).map((json) => Property(
      id: json['id'],
  ...
  // --- ADD NEW PROPERTY ---
  Future<void> addProperty(String ownerId, String name, String address, String city, String state, String pinCode, double rent, double deposit, {String propertyType = 'Flat', List<String>? photos}) async {
    try {
  ...
  // --- ADD NEW TENANT ---
  Future<void> addTenant(String propertyId, String name, String mobile, String email, String address, String pan, String dob, {String? aadhaar, String? emergencyContactName, String? emergencyContactNumber, String? moveInDate, String? moveOutDate}) async {
    try {
      await _supabase.from('tenants').insert({
        ...
        'move_out_date': (moveOutDate?.isEmpty ?? true) ? null : moveOutDate,
  ...
  // --- UPDATE PROPERTY ---
  Future<void> updateProperty(String propertyId, String ownerId, String name, String address, String city, String state, String pinCode, double rent, double deposit, {String propertyType = 'Flat', List<String>? photos}) async {
    try {
  ...
  Future<void> updatePropertyReminderSettings(String propertyId, String ownerId, bool enabled, int dueDay, String channel) async {
    try {
      await _supabase.from('properties').update({
  ...
  // --- UPDATE TENANT ---
  Future<void> updateTenant(String tenantId, String propertyId, String name, String mobile, String email, String address, String pan, String dob, {String? aadhaar, String? emergencyContactName, String? emergencyContactNumber, String? moveInDate, String? moveOutDate}) async {
    try {
  ...
  Future<void> _fetchAllLeads() async {
    ...
    leads = (response as List).map((json) => Lead.fromJson(Map<String, dynamic>.from(json))).toList();
    notifyListeners();
  }
  ...
  Future<void> _fetchAllClients() async {
    try {
      ...
      // Fetch Owners
      final ownersResponse = await _supabase.from('profiles').select().eq('role', 'owner');
      for (var json in (ownersResponse as List)) {
        allClients.add(Client(
          ...
          pan: json['pan'],
          address: json['address'],
          joinedDate: json['joined_date'] != null ? DateTime.tryParse(json['joined_date']) ?? DateTime.now() : DateTime.now(),
        ));
      }
      ...
      for (var json in (tenantsResponse as List)) {
        allClients.add(Client(
          ...
          joinedDate: json['created_at'] != null ? DateTime.tryParse(json['created_at']) ?? DateTime.now() : DateTime.now(),
        ));
      }
  ...
  Future<void> addClient(String name, String mobile, String email, String address, String role) async {
    try {
      if (role.toLowerCase() == 'owner') {
        ...
        await _supabase.from('profiles').insert({
          'id': '00000000-0000-0000-0000-111111111111', // In a real app, use uuid or auth
          'name': name,
          'mobile': mobile,
          'email': email.isEmpty ? null : email,
          'address': address,
  ...
  Future<void> addLead(String name, String mobile, String source, String status, String notes) async {
    try {
  ...
  Future<void> uploadAgreementDocument(String caseId, String fileName, Uint8List fileBytes) async {
    try {
      final path = '$caseId/$fileName';
      
      await _supabase.storage.from('agreements').uploadBinary(
        path,
        fileBytes,
        fileOptions: const FileOptions(upsert: true),
      );

      final String publicUrl = _supabase.storage.from('agreements').getPublicUrl(path);
  ...
  Future<void> createServiceRequest(String customerId, String serviceType, String propertyId, String tenantId, {Map<String, dynamic>? manualDetails}) async {
    try {
      final detailsJson = manualDetails ?? {
        'property_id': propertyId,
        'tenant_id': tenantId,
      };

      // 1. Insert Service Request
      final srResponse = await _supabase.from('service_requests').insert({
      ...
      if (detailsJson['is_existing_agreement'] == true) {
        if (propertyId.isNotEmpty) {
          final rentAmount = double.tryParse(detailsJson['existing_rent_amount'] ?? '0') ?? 0.0;
          final depositAmount = double.tryParse(detailsJson['existing_deposit_amount'] ?? '0') ?? 0.0;
          final payDate = int.tryParse(detailsJson['existing_rent_pay_date'] ?? '5') ?? 5;
          final endDate = detailsJson['existing_end_date'];
  ...
  // --- STORAGE & DOCUMENTS ---
  Future<String> uploadFileToBucket(String bucketName, String path, Uint8List fileBytes) async {
    try {
  ...
  Future<void> uploadPropertyPhoto(String propertyId, String fileName, Uint8List fileBytes) async {
    try {
      final path = '$propertyId/${DateTime.now().millisecondsSinceEpoch}_$fileName';
      final fileUrl = await uploadFileToBucket('properties', path, fileBytes);

      final property = properties.firstWhere((p) => p.id == propertyId);
      final List<String> currentPhotos = property.photos != null ? List.from(property.photos!) : [];
  ...
  Future<void> _fetchDocuments() async {
    try {
      final response = await _supabase.from('documents').select().order('uploaded_at', ascending: false);
      documents = (response as List).map((json) => Document.fromJson(Map<String, dynamic>.from(json))).toList();
      notifyListeners();
    } catch (e) {
      debugPrint('Error fetching documents: $e');
  ...
  Future<void> uploadDocument(String entityId, String entityType, String documentType, String fileUrl, String uploadedBy) async {
    try {
  ...
  Future<void> addPayment(String entityId, String entityType, double amount, String status, String paymentDate, String? transactionId, String description) async {
    try {
  ...
  Future<void> generateInvoice(String entityId, String entityType, double amount, String description) async {
    try {
      await _supabase.from('payments').insert({
        'entity_id': entityId,
        'entity_type': entityType,
